...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isHydrated || isLoading || (retryAfter && Date.now() < retryAfter)) return;
    ...
    try {
      if (isSignUp) {
        ...
        if (user) {
            ...
            window.location.href = postLoginPath(`/${role}/dashboard`);
        } else {
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={stat.desc} className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                  <stat.icon className="w-5 h-5 mb-2 text-white/90" aria-hidden="true" />
      ...
      <div className="flex-1 flex items-center justify-center bg-background p-6 lg:p-12">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="email" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 block">
                Email Address
              </label>
              <input
                id="email"
