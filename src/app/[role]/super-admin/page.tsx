...
export default function SuperAdminDashboard() {
  const [stats, setStats] = useState({ companys: 0, users: 0, mrr: 0 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
        ...
        // 2. Fetch Real Stats
        const { count: companyCount } = await supabase.from('companys').select('*', { count: 'exact', head: true });
        const { count: userCount } = await supabase.from('users').select('*', { count: 'exact', head: true });
        
        // Estimate MRR based on plans (Mock logic since we don't have subscription table yet)
        const { data: companys } = await supabase.from('companys').select('plan');
        let mrr = 0;
        companys?.forEach(t => {
    ...
    <div className="space-y-6 p-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Global overview of all companys and system health.</p>
        </div>
        <div className="flex gap-2">
            ...
            <Button asChild>
                <Link href="/super-admin/companys">Manage Companys</Link>
            </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Total Companys</p>
              <h3 className="text-2xl font-bold">{stats.companys}</h3>
        ...
        <Card>
          <CardContent className="pt-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Across all companys</p>
            </div>
