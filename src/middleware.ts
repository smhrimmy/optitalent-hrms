...
export function middleware(request: NextRequest) {
  ...
  const limit = (ip === '127.0.0.1' || ip === '::1') ? 5000 : 100; // Max requests per minute
  ...
  // If it's a company subdomain (not www or localhost), we pass it to the app
  if (subdomain && subdomain !== 'www' && subdomain !== 'localhost' && subdomain !== '127') {
      response.headers.set('x-company-slug', subdomain);
  }
  
  // Allow client to manually pass active company id if they have multiple memberships
  const manualCompanyId = request.headers.get('x-company-id');
  if (manualCompanyId) {
      response.headers.set('x-company-id', manualCompanyId);
  }
