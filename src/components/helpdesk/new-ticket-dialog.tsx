...
export function NewTicketDialog({ onNewTicket }: { onNewTicket: (ticket: Ticket) => void}) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!userData?.company_id || !userData.employees?.[0]?.id) throw new Error("Employee profile not found");

            // Upload file if exists
            let attachmentUrl = null;
            if (file) {
                const fileExt = file.name.split('.').pop();
                const fileName = `${userData.company_id}/${Math.random().toString(36).substring(2)}.${fileExt}`;
